  ...
  def _cache_function_parameters_and_return_type(self, tokenized_program):
    ...
    for line_num, line in enumerate(tokenized_program):
      if line and (line[0] == InterpreterBase.FUNC_DEF or line[0] == InterpreterBase.LAMBDA_DEF):
        ...
        isFunc = line[0] == InterpreterBase.FUNC_DEF
        func_name = line[1] if isFunc else self.create_lambda_name(line_num)
        paramsIndex = 2 if isFunc else 1 # lambdas don't have names
        params = [self._to_tuple(formal) for formal in line[paramsIndex:-1]]
